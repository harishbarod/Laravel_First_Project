...
<body>
    ...
    <div class="iphone">
        ...
        <form action="" class="form_data" id="form" method="POST">
          <div>
            <h2>Address</h2>
      
            <div class="card">
              <address style="color: black">
               404, girdhar colony <br />
          ...
          <fieldset style="margin-top: 10px">
            ...
            <div class="form__radios">
              ...
              <div class="form__radio">
                ...
                <input id="paypal" name="payment-method" type="radio"value="
                @if($data['payment_for_chat']=='chat') 
               
                {{ url('processpaypal_chat') }} @endif
                @if($data['payment_for_chat']!='chat') {{ url('processpaypal_cart') }} @endif
               "  />
              </div>

              <div class="form__radio " style="margin-bottom: 20px;">
                <label for="paypal"><svg class="icon">
                    <use xlink:href="#icon-paypal" />
                  </svg>Stripe</label>

                <input id="stripe" name="payment-method" type="radio" value="
                @if($data['payment_for_chat']=='chat') 
                {{-- {{ url('new_customer_payment_chat_one_time_payment') }}  --}}
                {{ url('stripe_chat') }} 
                
                @endif
                @if($data['payment_for_chat']!='chat') {{ url('stripe_cart') }}@endif"/>
              </div>
          ...
          <input type="hidden" name="payment_for_chat" value="{{ $data['payment_for_chat'] }}">
          <input type="hidden" name="plan_type" value="{{ $data['plan_type'] }}">
    

          <div>
            <button id="buy_button" onclick="changeUrl()" style=" margin-top:20px; " class="button button--full" type="submit"><svg class="icon" >
      ...
      function changeUrl(){
        ...
        var getUrl = document.querySelector('input[name = "payment-method"]:checked').value
        document.getElementById("form").action = getUrl;
        console.log(getUrl)
